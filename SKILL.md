...
1. 读完用户提供的材料（BP、PPT、memo、访谈记录、项目介绍均可）。
2. **内部**按 Q1–Q18 审计材料的覆盖情况——已覆盖 / 部分覆盖 / 未覆盖。
3. 对用户**只输出**一个轻量的初步判断：已经看清楚的方向 + 接下来可能聊的方向。不要在开头列 Part 覆盖表、缺口清单、投资人质疑清单或工作建议。
4. 按 Q1–Q18 顺序推进问答。材料已覆盖的固定问题不重复问；未覆盖的必须问；部分覆盖的围绕缺口改写后问。
5. 每次回答后，按 `references/02-follow-up-rules.md` 决定是否在当前 Part 内追问，以及追到什么深度。
...
2. 当前这一轮要问的一个问题。
3. 如果用户开了测试标记：问题前加一行 `测试标记：Part 编号 / 问题类型 / 触发原因`。
...
测试标记是上面规则的**唯一例外**，而且只在用户明确说"开测试标记"时使用。一旦用户没说开、或者说了关，测试标记就不要再加。
